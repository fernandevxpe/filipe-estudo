...
export default function DashboardHome() {
  ...
  const [authCloudRequired, setAuthCloudRequired] = useState(false);
  ...
  useEffect(() => {
    fetchApiJson<Record<string, unknown>>("/api/stats", undefined, "GET /api/stats").then((d) => {
      ...
      setAuthCloudRequired(Boolean((d as { authRequired?: boolean }).authRequired));
    ...
    <div className="space-y-8 w-full min-w-0">
      ...
      {authCloudRequired && (
        <div
          ...
          role="status"
        >
          <p className="font-medium text-amber-200">Progresso na nuvem</p>
          <p className="mt-1 text-amber-100/85">
            A app está configurada com Supabase: entra na tua conta para ver e guardar o progresso neste dispositivo.
          </p>
          <Link
            href="/login"
            className="inline-block mt-3 text-sm font-medium text-sky-300 hover:text-sky-200 underline underline-offset-2"
          >
